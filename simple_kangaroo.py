...
def kangaroo_search():
    ...
    for k in range(herd_size):
        ...
        W.append(secp256k1.point_addition(W0, secp256k1.scalar_multiplication(w[k])))
        dw.append(0)
    for k in range(herd_size):
        #wi.append(random.randint(1, (1 << (problem - 1))))
        wi.append(random.randint(1, (1 << (problem - 2))))
        Wi.append(secp256k1.point_addition(W1, secp256k1.scalar_multiplication(wi[k])))
        dwi.append(0)
    ...
    while (1):
        for k in range(herd_size):
            ...
            if solved: break
            t[k] += dt[k]
            T[k] = secp256k1.point_addition(P_table[pw], T[k])
        if solved: break            
        for k in range(herd_size):
            Hops += 1
            ...
            if solved: break
            w[k] += dw[k]
            W[k] = secp256k1.point_addition(P_table[pw], W[k])
        if solved: break
        for k in range(herd_size):
            Hops += 1
            ...
            if solved: break
            wi[k] += dwi[k]
            Wi[k] = secp256k1.point_addition(P_table[pw], Wi[k])
        if solved: break
        t1 = time.time()
        if (t1-t0) > 3:
...
inverse_find_point = secp256k1.point_addition(start_point, end_point)

W0 = secp256k1.pub2upub(search_pub)
W1 = secp256k1.point_subtraction(inverse_find_point, W0)
